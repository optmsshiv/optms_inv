...
async function downloadPDF() {
  ...
  // Mobile: use html2canvas + jsPDF for real download
  if (isMobile && typeof html2canvas !== 'undefined' && typeof jspdf !== 'undefined') {
    try {
      ...
      const hideEls = document.querySelectorAll('.sticky-bar,.upi-pay-btns,.wa-contact-btn,.pdf-btn,.pdf-dl-btn,.copy-btn,.lang-toggle,.ive-paid-btn,.partial-pay-box');
      hideEls.forEach(e => { e.dataset.wasVisible = e.style.display; e.style.display = 'none'; });

      const wrap   = document.querySelector('.wrap');
      const canvas = await html2canvas(wrap, {
        scale: 2, useCORS: true, allowTaint: true,
        backgroundColor: '#F5F6FA', logging: false,
        windowWidth: 700, width: 700
      });

      // Restore hidden elements
      hideEls.forEach(e => { e.style.display = e.dataset.wasVisible || ''; });
      ...
      while (yPos < imgH) {
        ...
        pdf.addImage(canvas.toDataURL('image/jpeg', 0.92), 'JPEG', margin, margin - (yPos * pageW / imgW), imgW, imgH);
        yPos += (pageH - 20) * (imgH / imgW) * (pageW / imgW);
      }

      pdf.save(filename);
      if (btn) { btn.innerHTML = '<i class="fas fa-check"></i> Downloaded!'; setTimeout(() => { btn.innerHTML = '<i class="fas fa-file-pdf"></i> Download as PDF'; btn.disabled = false; }, 2000); }
      return;
    } catch(err) {
  ...
  // Desktop fallback: use browser print → Save as PDF
